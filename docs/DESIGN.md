...
State 16-1-1 fixes the consumed-body release case: once a hold has been caught and its head/body segment has passed the receptor, that consumed segment never reappears after release. The remaining tail stays clamped to the receptor and uses the damaged-hold warning visual when its later sustain points miss. When that remaining tail reaches the receptor, it leaves the field whether its final point was held or missed.
